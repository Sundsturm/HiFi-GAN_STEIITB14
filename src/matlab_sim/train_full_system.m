...
%% --- CEK RANGE AKTIVASI ---
reset(mbq);
% Ambil satu sampel data dummy
[X_test, ~] = next(mbq); 

% Forward pass dan aktifkan tracing
x_in = dlarray(extractdata(X_test), 'TCB');
[~, state] = forward(netG, x_in); % State menyimpan nilai internal layer

% Loop semua layer state untuk cari nilai MAX/MIN absolut
max_val = 0;
all_tables = state.Value; % Mengambil tabel state

% Karena state mungkin cell array atau table, kita iterasi
names = state.Layer;
values = state.Value;

disp('--- MENCARI PEAK VALUE INTERNAL ---');
for i = 1:height(state)
    val = extractdata(values{i});
    current_max = max(abs(val(:)));
    
    if current_max > max_val
        max_val = current_max;
        disp(['New Max found at layer ' names{i} ': ' num2str(current_max)]);
    end
end

disp('-------------------------------------');
disp(['NILAI TERTINGGI DALAM NETWORK: ' num2str(max_val)]);

if max_val < 2.0
    disp('REKOMENDASI: Q2.14 (Sangat Presisi)');
elseif max_val < 4.0
    disp('REKOMENDASI: Q3.13 (Ideal)');
elseif max_val < 8.0
    disp('REKOMENDASI: Q4.12 (Aman)');
else
    disp('REKOMENDASI: Q5.11 (Wajib range besar)');
end
